...
var createTxObject = function (eosContractAddress, value, web3) {
  const gasLimitHex = web3.utils.toHex(300000); // 0x493e0 // what # to use as input?
  
  // Prepare a Raw Transaction object
  let gasPriceHex
  return web3.eth.getGasPrice()
    .then(gasPrice => { 
      gasPriceHex = web3.utils.toHex(gasPrice) // 10000000000
      return web3.eth.getTransactionCount(process.env.address) // getTransactionCountAsync
    })
    .then(transactionCount => {
      const rawTx = {
        nonce: transactionCount,
        gasPrice : gasPriceHex, // The gas price is determined by the x latest blocks median gas price.
        gasLimit: gasLimitHex, //'0x2710'
...
module.exports = {  
  ...
  sendEth2EosContract: async function (web3) {
    return createTxObject(eosContractAddress, 10000000, web3)
      .then(txInstance => {
        const tx = txInstance
        // console.log('\ntx instance of ethereumjs-tx \n', tx) //Transaction, raw, _fields, methods
        
        const txSerialized = signTxByPrivateKey(tx, process.env.PK) // makes it so funds are coming from the account address for that private key?
        return
